                    ...
                    if(it == y_tracking_list.begin()){
                        ...
                        if(new_w>=min_fsr_w && new_h>=min_fsr_h){
                            ...
                            if(size == target_size){
                                cls* cls_ptr = new cls;
                                cls_ptr->size = size;
                                cls_ptr->fsr_xmax = fsr_xmax;
                                cls_ptr->fsr_xmin = fsr_xmin;
                                cls_ptr->fsr_ymax = fsr_ymax;
                                cls_ptr->fsr_ymin = fsr_ymin;
...
void banking::run(){
    ...
    // 5. map clusters to multi-bit flip flops
    ...
    for(auto& ff_list: INST->ffs_sing){
        ...
        for(auto f: *ff_list){
            ...
            for(auto& sp: size_priority){
                if(sp.first == 1){
                    for(auto f: banking_ffs) ncls_ffs.push_back(f);
                    break;
                }  
                integra(sp.first);
                banking_ffs.clear();
            ...
            // start: map cluster to mbff
            cls_to_mbff();
            // end: map cluster to mbff
        }
